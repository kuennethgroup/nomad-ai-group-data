...
1. Drop any `.xlsx`/`.xls`/`.csv`/`.tsv` file into a NOMAD upload. A
   `TabularGuess` review entry is created automatically (`parsers/tabular_guess.py`).
...
3. Open the entry in the NOMAD GUI. Every guessed field is a normal
   ...
   a column, etc.
4. Choose a mapping mode, tick `confirm_schema`, and save. Column mode creates
   a native NOMAD schema plus one table entry with array quantities and scalar
   summaries. Row mode creates a native NOMAD schema plus one scalar archive
   entry per source table row.
5. Every entry generated this way carries a fixed `results.eln.tags` marker
   (`nomad_auto_upload_tables`), which the plugin's **Tabular Data** Explore
   app (`apps/__init__.py`) uses to bring all of them together in one
   dedicated tab, regardless of the per-upload schema/section name — filter
   by generated schema (via the "Generated schema" menu), by upload/author,
   or search by any guessed column value ("Column values").
...
- The "Tabular Data" Explore app (see above) gives one shared, generic view
  across all generated entries. It intentionally doesn't have per-schema
  columns/filters, since each upload can produce a differently named schema —
  use "Column values" in that app to search by a specific guessed quantity.
  Column mode is best for native table import and entry-level plots; row mode
  is best when each spreadsheet row should be searchable/visualizable as its
  own NOMAD entry.
